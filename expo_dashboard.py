...
class BoltTighteningApp(QtWidgets.QWidget):
    ...
    def _execute_robot_task(self):
        try:
            ...
            cycle = 0
            direction = 1          # +1 = tighten (+180°), -1 = loosen (-180°)
            cycles_in_direction = 0
            CYCLES_PER_DIRECTION = 5

            while not self.abort_flag:
                cycle += 1
                cycles_in_direction += 1
                dir_label = f"+180° (TIGHTEN)" if direction > 0 else f"-180° (LOOSEN)"
                self.log(f"\n========== CYCLE {cycle}  |  {dir_label}  [{cycles_in_direction}/{CYCLES_PER_DIRECTION}] ==========")
                    ...
                    if success and not self.abort_flag:
                        ...
                        if chk[0] != 0:
                            ...
                            if depth_below < 20.0:
                                ...
                                if not self.abort_flag:
                                    self.log(f"[SYS] Rotating J6 {'+' if direction > 0 else ''}{ 180 * direction}°...")
                                    res_j = self.robot.get_joint_position()
                                    if res_j[0] == 0:
                                        spin_target = list(res_j[1])
                                        spin_target[5] += math.radians(180 * direction)
                ...
                if not self.abort_flag:
                    if cycles_in_direction >= CYCLES_PER_DIRECTION:
                        direction = -direction
                        cycles_in_direction = 0
                        next_label = "+180° (TIGHTEN)" if direction > 0 else "-180° (LOOSEN)"
                        self.log(f"[SYS] 5 cycles complete — switching to {next_label}")
                    self.log(f"[SYS] Cycle {cycle} complete. Restarting sequence...")
